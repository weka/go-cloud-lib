...
func (j *JoinScriptGenerator) GetJoinScript(ctx context.Context) string {
	...
	IFS=$'\n' interfaces=($(ls /sys/class/net | grep -v lo | grep -v docker0 | grep -v enP))
	...
	if [[ $INSTALL_DPDK == true ]]; then
		getNetStrForDpdk $interface_num $(($DRIVES+$interface_num)) $interface_str "$GATEWAYS"
		...
		getNetStrForDpdk $(($interface_num+$DRIVES)) $(($interface_num+$DRIVES+$COMPUTE)) $interface_str "$GATEWAYS"
	...
	if [[ $FRONTEND -gt 0 ]]; then
		...
		if [[ $INSTALL_DPDK == true ]]; then
			getNetStrForDpdk $(($interface_num+$DRIVES+$COMPUTE)) $(($interface_num+$DRIVES+$COMPUTE+1)) $interface_str "$GATEWAYS"
