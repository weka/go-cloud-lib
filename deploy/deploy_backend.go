...
func (d *DeployScriptGenerator) GetBackendDeployScript() string {
	...
	waitForInterfaceIp ${all_interfaces[0]}
	...
	script := fmt.Sprintf(
		...
		gateways, clusterizeFunc, protectFunc, reportFunc, getCoreIdsFunc, getNetStrForDpdkFunc, getAllInterfaces, waitForInterfaceIp, d.DeviceNameCmd,
