...
func (d *DeployScriptGenerator) GetBackendDeployScript() string {
	...
	interfaces=$(ls /sys/class/net | grep -v lo | grep -v docker0 | grep enP | tail -n+2)
	...
	if [[ $FRONTEND_CONTAINER_CORES_NUM -gt 0 ]]; then
		...
		if [[ $INSTALL_DPDK == true ]]; then
			getNetStrForDpdk $(($interface_num+$DRIVE_CONTAINER_CORES_NUM+$COMPUTE_CONTAINER_CORES_NUM)) $(($interface_num+$NICS_NUM-1)) $interface_str "$GATEWAYS" "$SUBNETS"
